...
- **账单三类型（2026-08-23，对齐博客 bill-adapter 净资产口径）**：type 枚举 `expense/income/liability`（transfer 无数据不用）。类型判定显式优先：首词类型前缀 `支出/消费/花费→expense`、`收入→income`、`负债/借款/还款→liability`（须后跟空格，`消费30` 无空格不剥前缀）；**首词也可是分类白名单名**（`餐饮 午餐30`）或组合（`支出 餐饮 午餐30`，类型剥掉后再查一次分类）；无前缀时按关键词——收入（工资/收入/到账…）> 负债（借款/欠款/负债/花呗/白条 或 还款/偿还/还了）> 默认支出。**分类自动识别**靠 `_BILL_CATEGORY_KEYWORDS` 关键词表（午餐→餐饮、打车→交通、奖金→职业收入…），识别不到落「其他」再被 `_apply_bill_defaults` 换成配置默认；**发布前可手动纠正**：会话内发 `分类:白名单名` / `账户:xxx` 修改（白名单校验，liability 分类固定「负债」不可改，bill_batch 批量不支持逐条改）。**liability 符号特殊**：借入为正（新增负债）、还款为负（减少负债），不取绝对值（前端 `liability += amount` 累计，展示 `Math.max(0, liability)`）；liability 固定 `category: 负债`（tags 由 build_bill_md 自动 `["负债"]`），不套默认分类/账户；账户别名表 `花呗/白条/信用卡`（对齐现有 `account: 花呗` 数据，别名不参与标题清理）。回复文案带类型标签（支出/收入/负债）

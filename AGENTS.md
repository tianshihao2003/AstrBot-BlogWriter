...
- **账单两类型（2026-09-05 v1.0.50，对齐博客去负债后的纯收支模型）**：type 枚举只有 `expense/income`。类型判定显式优先：首词类型前缀 `支出/消费/花费→expense`、`收入→income`、`还款→expense（category 固定「还款」）`（须后跟空格，`消费30` 无空格不剥前缀）；**首词也可是分类白名单名**（`餐饮 午餐30`）或组合（`支出 餐饮 午餐30`，类型剥掉后再查一次分类）；无前缀时按关键词——收入（工资/收入/到账…）> 还款（还款/偿还/还了/已还 → expense、category「还款」、amount 恒为负）> 默认支出；「借款/欠款/欠了/负债」命中 `_BILL_BORROW_KEYWORDS` 直接报错「负债类型已下线」（博客 zod schema 已不含该类型，生成即构建失败）。**分类自动识别**靠 `_BILL_CATEGORY_KEYWORDS` 关键词表（午餐→餐饮、打车→交通、奖金→职业收入…），识别不到落「其他」再被 `_apply_bill_defaults` 换成配置默认；**发布前可手动纠正**：会话内发 `分类:白名单名` / `账户:xxx` 修改（白名单校验，bill_batch 批量不支持逐条改）。账户别名表 `花呗/白条/信用卡`（对齐现有 `account: 花呗` 数据，别名不参与标题清理）——账户仅是记账口径，花呗消费记 expense 正常统计。回复文案带类型标签（支出/收入）

...
- ✅ **自然语言正则记账/建日程**：`/账单 今天午餐微信花了32` / `/账单 发工资12000 银行卡` / `/日程 明天下午3点高优在会议室A开周会 每周重复 提前15分钟`，支持一句多笔账单、一句多个生日（含农历，生日按 `isLunar/lunarMonth/lunarDay` 存储、文件名 `lunar-M-D` 前缀）
...
| `/账单 午餐微信花了32` / `/账单` | 自然语言正则记账，支持口语 `今天午餐微信花了32` / `发工资12000 银行卡`，自动抽取金额/分类/账户/时间；一句多笔如 `午餐30晚餐45打车12` 自动批量 |
| `/日程 明天15点开周会 @会议室A 高优 每周 提前15分钟` / `/日程` | 自然语言正则建日程，到点微信提醒；一句多个生日如 `我的生日农历8.24对象12.22妈8.7都是农历` 自动批量（农历按 `lunar-M-D` 文件名存储） |
...
106 个单元测试 + 集成冒烟测试（stub astrbot 依赖），覆盖命令解析、markdown 生成（含 YAML 边界：时间不加引号、纯数字标签加引号、2026-08 新格式引号/行内数组）、图床/高德/GitHub 响应解析、友链字段识别、账单/日程正则解析（含批量与农历生日）、消息权限与放行逻辑。

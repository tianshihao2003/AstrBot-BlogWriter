...
- ✅ **自然语言正则记账 + 支出/收入两类型**（2026-09-05 对齐博客去负债后的模型）：`/账单 今天午餐微信花了32`（支出）/ `/账单 发工资12000 银行卡`（收入）；首词可显式指定类型 `支出/收入` 和分类白名单名（如 `/账单 食品酒水 买了箱牛奶45`、组合 `/账单 支出 餐饮 午餐30`）；分类自动识别靠关键词表（午餐→餐饮、打车→交通、奖金→职业收入…），识别不到可发 `分类:人情收礼`、`账户:现金` 在发布前手动纠正；支持一句多笔 `午餐30晚餐45打车12`；自动识别花呗/白条/信用卡账户（账户仅是记账口径）；负债/借款不再支持（命中提示下线；若内容提到还款仅按普通支出解析、分类「还款」，不再单独提示），与博客累计结余卡片联动

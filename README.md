...
| `/账单 午餐微信花了32` | 正则识别后**确认向导**：1.支出 2.收入 3.负债-借入 4.负债-还款 5.直接发布 6.重说 7.信用购；分类/账户已识别的直接跳过不重复问 |
| `/账单 信用购 花呗 午餐30` | **信用购一键两笔**：花呗/白条买东西不再记两次，`/发布` 一次写入 支出`-30`（账户花呗）+ 负债`+30`（花呗消费） |
| `/账单 负债 花呗借款5000` / `/账单 花呗还款2000` | 显式类型前缀；负债：借入正数、还款负数；一句多笔自动批量（批量不走向导） |
...
160 个单元测试 + 集成冒烟测试（stub astrbot 依赖），覆盖命令解析、markdown 生成（含 YAML 边界与最新格式）、图床/高德/GitHub/TMDB/xxapi 响应解析、友链字段识别、账单三类型 + 信用购拆两笔、向导全流程（笔记/相册/影视类型/账单确认，含选项错位与字段跳过回归）、日程/生日/纪念日解析（含农历与防呆）、影视（TMDB/手动/换封面/game 落盘）与书籍全流程、导航全流程、消息权限与放行逻辑。

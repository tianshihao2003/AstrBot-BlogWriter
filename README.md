...
- ✅ 微信对话直接发布动态 / 笔记 / 足迹 / 友链 / 相册 / 账单 / 日程
- ✅ **AI 自然语言记账/建日程**：绑定 AstrBot 模型，一句口语 `午餐微信花了32` / `发工资12000` / `明天下午3点高优在会议室A开周会 每周重复 提前15分钟` 自动抽取，失败自动正则兜底
- ✅ **日程微信提醒**：到点前 `N` 分钟（可自然语言“提前15分钟”或设置默认）微信私聊 `🔔 提醒`，`apscheduler` 持久化，重启不丢，支持 `/提醒 列表/取消`
...
| `/账单 午餐微信花了32` / `/账单` | 自然语言记账，支持口语 `今天吃了一顿饭花了30` / `发工资12000 银行卡`，AI 抽取金额/分类/账户/时间 |
| `/日程 明天15点开周会 @会议室A 高优 每周 提前15分钟` / `/日程` | 自然语言建日程，支持口语建今天日程定时间，到点微信提醒 |

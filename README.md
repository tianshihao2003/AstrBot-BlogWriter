...
- ✅ **自然语言正则记账 + 支出/收入两类型**（2026-09-05 对齐博客去负债后的模型）：`/账单 今天午餐微信花了32`（支出）/ `/账单 发工资12000 银行卡`（收入）/ `/账单 花呗还款2000`（还款自动记为支出，分类「还款」）；首词可显式指定类型 `支出/收入/还款` 和分类白名单名（如 `/账单 食品酒水 买了箱牛奶45`、组合 `/账单 支出 餐饮 午餐30`）；分类自动识别靠关键词表（午餐→餐饮、打车→交通、奖金→职业收入…），识别不到可发 `分类:人情收礼`、`账户:现金` 在发布前手动纠正；支持一句多笔 `午餐30晚餐45打车12`；自动识别花呗/白条/信用卡账户（账户仅是记账口径）；负债/借款不再支持（命中提示下线），与博客累计结余卡片联动
...
| `/账单 午餐微信花了32` | 正则识别后**确认向导**：1.支出 2.收入 3.直接发布 4.重说；分类/账户已识别的直接跳过不重复问 |
| `/账单 花呗还款2000` | 还款自动记为支出（分类「还款」、金额为负）；`/账单 还款 2000 微信` 显式前缀同效 |

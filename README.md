...
- ✅ **自然语言正则记账**：`/账单 今天午餐微信花了32` / `/账单 发工资12000 银行卡`，支持一句多笔如 `午餐30晚餐45打车12` 自动批量
...
116 个单元测试 + 集成冒烟测试（stub astrbot 依赖），覆盖命令解析、markdown 生成（含 YAML 边界：时间不加引号、纯数字标签加引号、2026-08 新格式引号/行内数组）、图床/高德/GitHub 响应解析、友链字段识别、账单/日程正则解析（含批量与农历生日、纪念日、防呆拦截）、消息权限与放行逻辑。
